import importlib.util
import tempfile
import unittest
from pathlib import Path
from unittest.mock import patch

ROOT = Path(__file__).resolve().parents[1]
spec = importlib.util.spec_from_file_location("install_codex", ROOT / "scripts/install_codex.py")
installer = importlib.util.module_from_spec(spec)
spec.loader.exec_module(installer)


class InstallationTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory()
        self.addCleanup(self.temp.cleanup)
        self.destination = Path(self.temp.name) / "skills with spaces"

    def test_dry_run_writes_nothing(self):
        targets = installer.install(self.destination, True)
        self.assertEqual(len(targets), 7)
        self.assertFalse(self.destination.exists())

    def test_every_installation_is_self_contained(self):
        targets = installer.install(self.destination)
        expected = (installer.PLUGIN / "reference/decision-brief.md").read_bytes()
        for target in targets:
            with self.subTest(skill=target.name):
                self.assertTrue((target / "SKILL.md").is_file())
                self.assertEqual((target / "references/decision-brief.md").read_bytes(), expected)
                source = installer.PLUGIN / "skills" / target.name
                for original in source.rglob("*"):
                    if original.is_file():
                        self.assertEqual((target / original.relative_to(source)).read_bytes(), original.read_bytes())

    def test_existing_skill_stops_before_other_skills_are_written(self):
        existing = self.destination / "ci-triage"
        existing.mkdir(parents=True)
        (existing / "SKILL.md").write_text("user changes")
        with self.assertRaises(FileExistsError):
            installer.install(self.destination)
        self.assertEqual((existing / "SKILL.md").read_text(), "user changes")
        self.assertEqual(list(self.destination.iterdir()), [existing])

    def test_dangling_symlink_is_preserved(self):
        self.destination.mkdir()
        existing = self.destination / "ci-weekly"
        existing.symlink_to(self.destination / "absent")
        with self.assertRaises(FileExistsError):
            installer.install(self.destination)
        self.assertTrue(existing.is_symlink())
        self.assertEqual(list(self.destination.iterdir()), [existing])

    def test_copy_failure_removes_partial_skills(self):
        self.destination.mkdir()
        unrelated = self.destination / "user-skill.txt"
        unrelated.write_text("keep")
        with patch.object(installer.shutil, "copy2", side_effect=OSError("copy failed")):
            with self.assertRaises(OSError):
                installer.install(self.destination)
        self.assertEqual(list(self.destination.iterdir()), [unrelated])

    def test_second_install_does_not_modify_first(self):
        installer.install(self.destination)
        before = {p.relative_to(self.destination): p.read_bytes() for p in self.destination.rglob("*") if p.is_file()}
        with self.assertRaises(FileExistsError):
            installer.install(self.destination)
        after = {p.relative_to(self.destination): p.read_bytes() for p in self.destination.rglob("*") if p.is_file()}
        self.assertEqual(before, after)


if __name__ == "__main__":
    unittest.main()
